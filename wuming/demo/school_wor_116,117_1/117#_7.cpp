...
{
	if (year % 4 == 0 || year % 100 == 0)//用if语句判断是否为闰年
	{
		return 1;
	}
	return 0;
}
int Num_year(int year,int M)//输出在指定年份时相应月份的天数，接收并创建year，M的形参
{
	int month = 0;//创建用于接收返回数值的临时变量
	...
	return month;//临时变量month在switch的语句当中已被赋值，返回month变量就可知该月天数
}
int main()
{
	int year = 0;
	...
	{
		printf("本月天数为%d天", Num_year(year,M));//调用月份输出函数，将year,M传值到Num_year函数当中
	}
	else
	{
